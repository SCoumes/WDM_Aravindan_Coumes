...
def queryFunction(title, author):
    ...
    if('organic_results' in results.keys()):
        if('publication_info' in results['organic_results'][0].keys()):
            if('authors' in  results['organic_results'][0]['publication_info'].keys()):
                    ...
                    for each in Author_Ids:
                        ...
                        search = GoogleSearch(params)
                        results = search.get_dict()
                        if( 'affiliations' in results['author'].keys()):
